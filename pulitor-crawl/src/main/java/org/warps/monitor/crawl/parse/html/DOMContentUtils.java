...
import org.warps.pulsar.common.NodeWalker;
...
public class DOMContentUtils {
  ...
  public static final Logger LOG = LoggerFactory.getLogger("org.warps.pulsar.parse.html");
  ...
  public URL getBaseURLFromTag(Node node) {
    NodeWalker walker = new NodeWalker(node);
  ...
  public ArrayList<Outlink> getOutlinks(URL base, Node root) {
    return getOutlinks(base, new ArrayList<>(), root);
  }

  public ArrayList<Outlink> getOutlinks(URL base, ArrayList<Outlink> outlinks, Node root) {
    NodeWalker walker = new NodeWalker(root);

    while (walker.hasNext()) {
      Node currentNode = walker.nextNode();
      getOutlinksStep2(base, outlinks, currentNode);
      walker.skipChildren();
    }
  ...
  private void getOutlinksStep2(URL base, ArrayList<Outlink> outlinks, Node node) {
    NodeWalker walker = new NodeWalker(node);
